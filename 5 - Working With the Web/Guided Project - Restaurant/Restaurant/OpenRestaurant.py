...
# Disabled normal command-line functionality.
...
def resolve_url(menu_item):
	image_name = menu_item.pop('imageName')
	menu_item['image_url'] = 'http://' + request.environ['HTTP_HOST'] + '/images/' + image_name
	return menu_item
...
def categories():
	...
	return {
		'categories': uniq([ item['category'] for item in menu ])
	}
...
def category():
	...
	menu = json.load(open('menu.json'))
	menu_with_urls = [resolve_url(item) for item in menu]
	return {
		'items': [ item for item in menu_with_urls if item['category'] == request.query.category ]
	}

@route('/images/<image_filename>')
def image(image_filename):
	images_dir = abspath('Images/')
	return static_file(image_filename, root=images_dir)
