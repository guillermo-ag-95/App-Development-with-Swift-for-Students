...
func mealTakenFor(_ time: String) -> Meal? {
    var result: Meal? = nil
    
    if let meal = meals[time] {
        result = meal
    }
...
if let meal = mealTakenFor("Breakfast") {
    print(meal)
}

if let meal = mealTakenFor("Dinner") {
    print(meal)
}
/*:
 ...
 */
func retrieveLog(mealLogDBKey: String) -> [String: Meal]? {
    var result: [String: Meal]? = nil
    
    if let meal = UserDefaults.standard.dictionary(forKey: mealLogDBKey) {
        result = (meal as! [String : Meal])
    }
    
    return result
}

if let meal = retrieveLog(mealLogDBKey: "someKey") {
    print(meal)
}
